...
hydro_hp = np.sqrt(c.k_B.cgs.value * hydro_T * hydro_r**3 / (2.3 * c.m_p.cgs.value * c.G.cgs.value * M_star))
...
def fit_wrapper(x, M1, M2, M3, R1, R2, R3, alpha):
    M1 *= M_Jup
    M2 *= M_Jup
    M3 *= M_Jup
    R1 *= au
    R2 *= au
    R3 *= au
    return get_fit(x, sig_0, 1.0, [M1, M2, M3], [R1, R2, R3], alpha, hp, M_star)
...
p0 = [0.10, 1.0, 1.0, 65, 105, 161, 1e-3]
bounds = (
    [0, 0, 0, 1.0, 1.0, 1.0, 1e-5],
    [5, 5, 5, 300, 300, 300, 1e-1])
...
sliderAlpha = Slider(axAlpha, r'$\log\alpha$', -4, -1, valinit=-3, valfmt='%.2f')
...
def callback(event):
    ...
    sig_fit = get_fit(r, sig_0, time, [M1, M2, M3], [R1, R2, R3], alpha, hp, M_star)
